...
def filter_search(df, keyword, year_min=2000, year_max=2019, donor='China', Active=True):
    ...
    result = df[
        df['description'].str.contains(keyword, na=False, case=False) | df['title'].str.contains(keyword, na=False,
                                                                                                 case=False)].reset_index(
        drop=True)
    return result
...
def write_useful_resources():
    ...
    st.markdown(
        "[TUFF 1.4 Guide](https://docs.google.com/document/d/1henyi4ixkRKMSH4k2Is9g-N5T8GeBWgEOINDzSaLE4c/edit)")
    st.markdown(
        "[CRS Sector](https://www.oecd.org/dac/stats/documentupload/Budget%20identifier%20purpose%20codes_EN_Apr%202016.pdf)")
